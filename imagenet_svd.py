...
INPUT_DATA = 'ILSVRC2012_img_train_t3/'
...
def run_bottleneck_on_image(sess, image_data, image_data_tensor, bottleneck_tensor, category):
    ...
    global A, A_row_number, test_start_row
    A = np.r_[A, np.array(bottleneck_values).reshape(1, 2048)]
    A_row_number = A_row_number + 1
    if test_start_row == 0 and category == 'testing':
        test_start_row = A_row_number
...
def get_random_cached_bottlenecks(sess, n_classes, image_lists, how_many, category,
    ...
    for _ in range(how_many):
        ...
        if B_row_number < A_row_number:
            B = np.r_[B, np.array(ground_truth).reshape(1, 120)]
            B_row_number = B_row_number + 1
    return bottlenecks, ground_truths

def get_random_cached_bottlenecks_svd(A, B, train_or_validate):
    bottlenecks = []
    ground_truths = []
    train_or_validate_curve = round(0.875*test_start_row)
    for i in range(BATCH):
        if train_or_validate == 0:
            x = random.randint(1, train_or_validate_curve-1)
        if train_or_validate == 1:
            x = random.randint(train_or_validate_curve, test_start_row - 1)
        bottlenecks.append(A[x])
        ground_truths.append(B[x])
    return bottlenecks, ground_truths
...
def main():
    ...
    with tf.Session() as sess:
        ...
        #svd 50
        bottlenecks_after_svd_50 = []
        s1 = np.zeros((len(A), 2048), dtype=float)
        u1,sigma1,vt1 = la.svd(A)
        for i in range(50):
            s1[i][i] = sigma1[i]
        tmp = np.dot(u1, s1)
        A1 = np.dot(tmp,vt1)
        ...
        for i in range(STEPS):
            # 每次获取一个batch的训练数据
            train_bottlenecks, train_ground_truth = get_random_cached_bottlenecks_svd(A1, B, 0)
            sess.run(train_step,
                     feed_dict={bottleneck_input: train_bottlenecks, ground_truth_input: train_ground_truth})
            if i % 100 == 0 or i + 1 == STEPS:
                validation_bottlenecks, validation_ground_truth = get_random_cached_bottlenecks_svd(A1, B ,1)
                validation_accuracy = sess.run(evaluation_step, feed_dict={
                    bottleneck_input: validation_bottlenecks, ground_truth_input: validation_ground_truth})
                print('Step %d: svd 50 Validation accuracy on random sampled %d examples = %.1f%%'
                      % (i, BATCH, validation_accuracy * 100))

        x = test_start_row
        while x < len(A1):
            bottlenecks_after_svd_50.append(A1[x])
            x = x + 1

        test_accuracy = sess.run(evaluation_step, feed_dict={bottleneck_input: bottlenecks_after_svd_50,
                                                             ground_truth_input: test_ground_truth})
        print('Final test accuracy (svd 50) = %.1f%%' % (test_accuracy * 100))

        # svd 25
        bottlenecks_after_svd_25 = []
        s2 = np.zeros((len(A), 2048), dtype=float)
        u2, sigma2, vt2 = la.svd(A)
        for i in range(25):
            s2[i][i] = sigma2[i]
        tmp = np.dot(u2, s2)
        A2 = np.dot(tmp, vt2)
        ...
        for i in range(STEPS):
            # 每次获取一个batch的训练数据
            train_bottlenecks, train_ground_truth = get_random_cached_bottlenecks_svd(A2, B, 0)
            sess.run(train_step,
                     feed_dict={bottleneck_input: train_bottlenecks, ground_truth_input: train_ground_truth})
            if i % 100 == 0 or i + 1 == STEPS:
                validation_bottlenecks, validation_ground_truth = get_random_cached_bottlenecks_svd(A2, B, 1)
                validation_accuracy = sess.run(evaluation_step, feed_dict={
                    bottleneck_input: validation_bottlenecks, ground_truth_input: validation_ground_truth})
                print('Step %d: svd 25 Validation accuracy on random sampled %d examples = %.1f%%'
                      % (i, BATCH, validation_accuracy * 100))

        x = test_start_row
        while x < len(A2):
            bottlenecks_after_svd_25.append(A2[x])
            x = x + 1

        test_accuracy = sess.run(evaluation_step, feed_dict={bottleneck_input: bottlenecks_after_svd_25,
                                                             ground_truth_input: test_ground_truth})
        print('Final test accuracy (svd 25) = %.1f%%' % (test_accuracy * 100))
