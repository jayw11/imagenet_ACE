...
def get_or_create_bottleneck(sess, image_path, jpeg_data_tensor, bottleneck_tensor):
    ...
    # 返回得到的特征向量
    return bottleneck_values, node_id, cal_time
...
def get_test_bottlenecks(sess, jpeg_data_tensor, bottleneck_tensor):
    bottlenecks = []
    inception_classify = []
    image_path_list = []
    i = 0
    ...
    for k in range(TEST_SIZE):
        ground_truths[k][int(content[k])] = 1
    ...
    for image_path in image_path_list:
        ...
        bottleneck, node_id, cal_time = get_or_create_bottleneck(sess, image_path,
                                                                          jpeg_data_tensor, bottleneck_tensor)
        cal_bn_time = cal_bn_time + cal_time
        inception_classify.append(node_id)
    ...
    for j in range(TEST_SIZE):
        if int(content[j]) == inception_classify[j]:
            correct_classify_count = correct_classify_count + 1
    inception_accuracy = correct_classify_count / TEST_SIZE
...
def main():
    ...
    with tf.Session() as sess:
        ...
        test_bottlenecks, test_ground_truth, inception_accuracy = get_test_bottlenecks(sess,
                                                                                jpeg_data_tensor, bottleneck_tensor)
