...
def run_bottleneck_on_image(sess, image_data, image_data_tensor, bottleneck_tensor, category):
    ...
    global A, A_row_number
    A = np.r_[A, np.array(bottleneck_values).reshape(1, 2048)]
    A_row_number = A_row_number + 1
...
def get_fixed_size_bottlenecks(sess, n_classes, image_lists, how_many, category, jpeg_data_tensor, bottleneck_tensor):
    ...
    i = 0
    while i < how_many:
        if i != 0 and (i % 1000 == 0 or i == how_many - 1):
            print("loading ", i ," images")
        ...
        if not os.path.exists(bottleneck_path):
            ...
            if B_row_number < A_row_number:
                B = np.r_[B, np.array(ground_truth).reshape(1, 1000)]
...
def svd_test(sigular_value_number, sess, n_classes, image_lists,jpeg_data_tensor,
             bottleneck_tensor, train_step, evaluation_step, bottleneck_input,
             ground_truth_input,test_bottlenecks, test_ground_truth):
    #bottlenecks_t, ground_truth_t, bottlenecks_v, ground_truth_v = []
    ...
    for i in range(sigular_value_number):
        s[i][i] = sigma[i]
    tmp = np.dot(u, s)
    ...
    print('Final test accuracy (svd %d) = %.1f%%' % (sigular_value_number, test_accuracy * 100))
...
def main():
    ...
    with tf.Session() as sess:
        ...
        for i in range(STEPS):
            ...
            #print("training time: ", end_time_2 - start_time_2)
            if i%100 == 0 or i+1 == STEPS:
                validation_bottlenecks, validation_ground_truth = get_random_cached_bottlenecks(bottlenecks_v, ground_truth_v)
                validation_accuracy = sess.run(evaluation_step, feed_dict={
                    bottleneck_input:validation_bottlenecks, ground_truth_input: validation_ground_truth})
        ...
        #svd
        svd_test(1024, sess, n_classes, image_lists,jpeg_data_tensor,bottleneck_tensor, train_step, evaluation_step,
                 bottleneck_input, ground_truth_input,test_bottlenecks, test_ground_truth)
        svd_test(512, sess, n_classes, image_lists,jpeg_data_tensor,bottleneck_tensor, train_step, evaluation_step,
                 bottleneck_input, ground_truth_input,test_bottlenecks, test_ground_truth)
        svd_test(256, sess, n_classes, image_lists,jpeg_data_tensor,bottleneck_tensor, train_step, evaluation_step,
                 bottleneck_input, ground_truth_input,test_bottlenecks, test_ground_truth)
        svd_test(128, sess, n_classes, image_lists,jpeg_data_tensor,bottleneck_tensor, train_step, evaluation_step,
                 bottleneck_input, ground_truth_input,test_bottlenecks, test_ground_truth)
        svd_test(64, sess, n_classes, image_lists,jpeg_data_tensor,bottleneck_tensor, train_step, evaluation_step,
                 bottleneck_input, ground_truth_input,test_bottlenecks, test_ground_truth)
        svd_test(32, sess, n_classes, image_lists,jpeg_data_tensor,bottleneck_tensor, train_step, evaluation_step,
                 bottleneck_input, ground_truth_input,test_bottlenecks, test_ground_truth)
