...
A = np.zeros((100000, 2048), dtype=float)
B = np.zeros((100000, 1000), dtype=float)
A_row_number = 0
B_row_number = 0
...
def run_bottleneck_on_image(sess, image_data, image_data_tensor, bottleneck_tensor, category, i):
...
def run_bottleneck_on_image_no_index(sess, image_data, image_data_tensor, bottleneck_tensor, category):
    # 这个过程实际上就是将当前图片作为输入计算瓶颈张量的值。这个瓶颈张量的值就是这张图片的新的特征向量。
    bottleneck_values = sess.run(bottleneck_tensor, {image_data_tensor: image_data})
    # 经过卷积神经网络处理的结果是一个四维数组，需要将这个结果压缩成一个特征向量（一维数组）
    bottleneck_values = np.squeeze(bottleneck_values)
...
def get_or_create_bottleneck(sess, image_lists, label_name, image_index, category, jpeg_data_tensor, bottleneck_tensor):
    ...
    if not os.path.exists(bottleneck_path):
        ...
        #time_1 = time.clock()
        bottleneck_values = run_bottleneck_on_image_no_index(sess, image_data, jpeg_data_tensor, bottleneck_tensor, category)
        #time_2 = time.clock()
        # 将计算得到的特征向量存入文件
        ...
        #time_3 = time.clock()
        #print("calculate:", time_2-time_1, "write file:", time_3-time_2)
    else:
        # 直接从文件中获取图片相应的特征向量。
...
def get_fixed_size_bottlenecks(sess, n_classes, image_lists, how_many, category, jpeg_data_tensor, bottleneck_tensor):
    ...
    while i < how_many:
        ...
        if not os.path.exists(bottleneck_path):
            ...
            image_data = gfile.FastGFile(image_path, 'rb').read()
            time_1 = time.clock()
            bottleneck_values = run_bottleneck_on_image(sess, image_data, jpeg_data_tensor, bottleneck_tensor, category, i)
            time_2 = time.clock()
            bottleneck_string = ','.join(str(x) for x in bottleneck_values)
            with open(bottleneck_path, 'w') as bottleneck_file:
                bottleneck_file.write(bottleneck_string)
            ...
            time_4 = time.clock()
            global B, B_row_number, A_row_number
            if B_row_number < A_row_number:
                B[i] = np.array(ground_truth).reshape(1, 1000)
                B_row_number = B_row_number + 1
            i = i + 1
            time_5 = time.clock()
            time1 = time1 + time_1 - time_0
            time2 = time2 + time_2 - time_1
            time3 = time3 + time_3 - time_2
            time4 = time4 + time_5 - time_4
        if i != 0 and (i % 2000 == 0 or i == how_many - 1):
            print('load %d: images. get path time: %.3fs. cal bottleneck time: %.3fs. write file time: %.3fs. '
                  'add row time: %.3fs. ' % (i, time1, time2, time3, time4))
            time1 = 0
            time2 = 0
            time3 = 0
            time4 = 0
...
def svd_test(singular_value_number, n_classes, bottleneck_input, ground_truth_input,
            test_bottlenecks, test_ground_truth, bottlenecks_t, ground_truth_t, bottlenecks_v, ground_truth_v):
    ...
    s = np.zeros((len(bottlenecks_t), 2048), dtype=float)
    u, sigma, vt = la.svd(bottlenecks_t)
    for i in range(singular_value_number):
        s[i][i] = sigma[i]
    tmp = np.dot(u, s)
    bottlenecks_after_svd_t = np.dot(tmp, vt)
    ...
    with tf.Session() as sess2:
        ...
        for i in range(STEPS):
            # 每次获取一个batch的训练数据
            train_bottlenecks, train_ground_truth = get_random_cached_bottlenecks(bottlenecks_after_svd_t, ground_truth_t)
    ...
    # 在最后的测试数据上测试正确率
        test_accuracy = sess2.run(evaluation_step, feed_dict={bottleneck_input: test_bottlenecks,
...
def main():
    ...
    with tf.Session() as sess:
        tf.global_variables_initializer().run()
        bottlenecks_t, ground_truth_t = get_fixed_size_bottlenecks(sess, n_classes, image_lists,
        ...
        #global A, B, A_row_number, B_row_number
        #A = np.zeros((1, 2048), dtype=float)
        #B = np.zeros((1, 1000), dtype=float)
        #A_row_number = 0
        #B_row_number = 0
    ...
    #svd
    svd_test(1024, sess, n_classes, bottleneck_input, ground_truth_input,
             test_bottlenecks, test_ground_truth, bottlenecks_t, ground_truth_t, bottlenecks_v, ground_truth_v)
    svd_test(512, sess, n_classes, bottleneck_input, ground_truth_input,
             test_bottlenecks, test_ground_truth, bottlenecks_t, ground_truth_t, bottlenecks_v, ground_truth_v)
    svd_test(256, sess, n_classes, bottleneck_input, ground_truth_input,
             test_bottlenecks, test_ground_truth, bottlenecks_t, ground_truth_t, bottlenecks_v, ground_truth_v)
    svd_test(128, sess, n_classes, bottleneck_input, ground_truth_input,
             test_bottlenecks, test_ground_truth, bottlenecks_t, ground_truth_t, bottlenecks_v, ground_truth_v)
    svd_test(64, sess, n_classes, bottleneck_input, ground_truth_input,
             test_bottlenecks, test_ground_truth, bottlenecks_t, ground_truth_t, bottlenecks_v, ground_truth_v)
    svd_test(32, sess, n_classes, bottleneck_input, ground_truth_input,
             test_bottlenecks, test_ground_truth, bottlenecks_t, ground_truth_t, bottlenecks_v, ground_truth_v)
